...
const useQueryParams = () => {
  ...
  queryParams.forEach((value, key) => {
    if (!BASE_PARAMS.includes(key)) filters[key] = value;
  });
  ...
  const setQueryParams = (newParams) => {
    ...
    Object.entries(newParams).forEach(([key, value]) => {
      ...
      else newQuery.set(key, value.toString());
    });
    ...
    if (newParams.all_data === true) {
      ...
      if (Object.keys(newParams).some((k) => k !== "page")) newQuery.set("page", "1");
    }

    navigate(`${location.pathname}?${newQuery.toString()}`, { replace: true });
  };

  return { page, per_page, all_data, search, ordering, filters, setQueryParams };
};
