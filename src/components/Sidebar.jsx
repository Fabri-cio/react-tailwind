...
const SidebarMenu = React.memo(({ title, icon: Icon, items, isOpen, toggleMenu }) => (
    ...
    {isOpen && (
      <ul className="pl-4">
        {items.map((item, index) => (
          <li key={index} className="rounded hover:shadow hover:bg-blue-950 py-2">
      ...
      <ul className="mt-3 text-white font-bold">
        {menus.map((menu, index) => (
          <SidebarMenu
            ...
            toggleMenu={() => setOpenMenu(openMenu === menu.title ? null : menu.title)}
          />
        ))}
      </ul>
    </div>
